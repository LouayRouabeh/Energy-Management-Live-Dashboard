...
@app.callback(Output('saveSql1', 'children'), [
...
def update_graph(contents, filename):
    if contents:
        ...
        df.to_sql('floor2.db', con=engine, if_exists='replace')
...
@app.callback(
    dash.dependencies.Output('table-store2', 'children'),
...
@app.callback(
    dash.dependencies.Output('dropdown-x2', 'options'),
    [dash.dependencies.Input('table-store', 'children')])
...
@app.callback(
    dash.dependencies.Output('dropdown-y2', 'options'),
    [dash.dependencies.Input('table-store', 'children')])
...
def populate_datatable(n_intervals):
    df = pd.read_sql_table('floor2.db', con=engine)
    return [
        dash_table.DataTable(
            id='our-table',
            columns=[{
...
def df_to_csv(n_clicks, n_intervals, dataset, s):
    ...
    if input_triggered == "save_to_postgres":
        ...
        pg.to_sql('floor2.db', con=engine, if_exists='replace', index=False)
        return output, s
